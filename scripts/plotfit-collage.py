...
b_F = numpy.abs(fit['fittedparameters'][:, 0] / D)
b_Q = numpy.abs(fit['fittedparameters'][:, 1] / D)
...
for i in range(0, len(dirs), 10):
    ...
    for dir in chunk:
        plot(dir, ax)
    ax[0, 0].text(0.3, 0.1, 'QQ', transform=ax[0,0].transAxes)
    ax[1, 0].text(0.3, 0.1, 'QF', transform=ax[1,0].transAxes)
    ax[2, 0].text(0.3, 0.1, 'FF', transform=ax[2,0].transAxes)

    canvas = FigureCanvasAgg(fig)
    fig.savefig('bestfit-%s.png' % dirs[i])
    fig.savefig('bestfit-%s.svg' % dirs[i])
